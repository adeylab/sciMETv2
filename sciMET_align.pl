...
-x           Do not proceed with sorting and merging (if -1 and -2)
...
if (defined $opt{'1'}) {
	...
	$r1_align = "bismark --path_to_bowtie $bowtie -o $opt{'O'}.R1 --pbat -p $threads $opt{'1'} >> $opt{'O'}.R1.align.log 2>> $opt{'O'}.R1.align.log";
	print LOG "\t$r1_align\n";
	system($r1_align);
...
if (defined $opt{'2'}) {
	...
	$r2_align = "bismark --path_to_bowtie $bowtie -o $opt{'O'}.R2 -p $threads $opt{'2'} >> $opt{'O'}.R2.align.log 2>> $opt{'O'}.R2.align.log";
	print LOG "\t$r2_align\n";
	system($r2_align);
...
if (defined $opt{'1'} && defined $opt{'2'} && !defined $opt{'x'}) {
	$ts = localtime(time);
	print LOG "$ts Name sorting reads and merging...\n";
	$threads2 = $threads*2;
	...
	print LOG "\t$merge\n";
	if (defined $opt{'D'}) {
		system("rm -f -R $opt{'O'}.merge.log $opt{'O'}.R1/ $opt{'O'}.R2/");
	}
}
